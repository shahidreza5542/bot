...
async function fetchMeme(subreddit = 'memes') {
  try {
    const response = await axios.get(`https://meme-api.com/gimme/${subreddit}`, { timeout: 5000 });
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('subreddit').setDescription('Subreddit to get meme from').setRequired(false)
        .addChoices({ name: 'Memes', value: 'memes' }, { name: 'DankMemes', value: 'dankmemes' }, { name: 'Funny', value: 'funny' }, { name: 'ProgrammerHumor', value: 'ProgrammerHumor' }))
    .addStringOption(option =>
      option.setName('delivery').setDescription('How to deliver the joke').setRequired(false)
        .addChoices({ name: 'Just me', value: 'personal' }, { name: 'Mention everyone', value: 'everyone' })),
  ...
  async execute(interaction) {
    ...
    const previewImage = meme.preview && meme.preview.length > 0 ? meme.preview[Math.floor(meme.preview.length / 2)] : meme.url;

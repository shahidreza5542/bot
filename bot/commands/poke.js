...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (targetUser.id === interaction.user.id) return interaction.reply({ content: 'You poked yourself! Hey, wake up!', ephemeral: true });

    const embed = await createActionEmbed({
      title: 'Poke!',
      description: `<@${interaction.user.id}> poked <@${targetUser.id}>! Hey!`,
      color: 0x00CED1,
      gifType: 'poke',

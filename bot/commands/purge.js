...
module.exports = {
  data: new SlashCommandBuilder().setName('purge').setDescription('Delete messages (Admin Only)')
  ...
  async execute(interaction) {
    ...
    if (targetUser) {
      ...
      if (userMessages.size === 0) return interaction.reply({ content: 'No messages found from this user', ephemeral: true });
    ...
    const embed = new EmbedBuilder().setTitle('Messages Deleted').setDescription(`Deleted ${deletedCount} messages`).setColor(0x4F46E5).setTimestamp();

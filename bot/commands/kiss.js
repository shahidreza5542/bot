...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (targetUser.id === interaction.user.id) return interaction.reply({ content: 'You kissed yourself! Self-love wins!', ephemeral: true });

    const embed = await createActionEmbed({
      title: 'Kiss!',
      description: `<@${interaction.user.id}> gave <@${targetUser.id}> a sweet kiss!`,
      color: 0xFF1493,
      gifType: 'kiss',

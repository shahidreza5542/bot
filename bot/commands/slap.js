...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (targetUser.id === interaction.user.id) return interaction.reply({ content: 'You slapped yourself! That must have hurt!', ephemeral: true });

    const embed = await createActionEmbed({
      title: 'Slap!',
      description: `<@${interaction.user.id}> slapped <@${targetUser.id}>! Ouch!`,
      color: 0xFF4500,
      gifType: 'slap',

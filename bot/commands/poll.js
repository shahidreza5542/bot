...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const options = [];
    for (let i = 1; i <= 4; i++) { const option = interaction.options.getString(`option${i}`); if (option) options.push(option); }

    const emojis = ['1️⃣', '2️⃣', '3️⃣', '4️⃣'];
    let description = '';
    options.forEach((opt, idx) => { description += `${emojis[idx]} ${opt}\n`; });

    const embed = new EmbedBuilder().setTitle(`${question}`).setDescription(description).setColor(0x5865F2)
      .setFooter({ text: `Poll by ${interaction.user.tag} | React to vote!` }).setTimestamp();

...
module.exports = {
  data: new SlashCommandBuilder().setName('rank').setDescription('View your or another user\'s rank')
    .addUserOption(option => option.setName('user').setDescription('User to check rank for').setRequired(false)),
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const embed = new EmbedBuilder().setTitle(`${targetUser.username}'s Rank`).setThumbnail(targetUser.displayAvatarURL({ dynamic: true }))
        .addFields(
          { name: 'Rank', value: `#${rank}`, inline: true }, { name: 'Level', value: levelData.level.toString(), inline: true },
          { name: 'XP', value: levelData.xp.toString(), inline: true }, { name: 'Messages', value: (levelData.messages || 0).toString(), inline: true },
          { name: 'Progress', value: `${progressBar} ${progressPercent}%`, inline: false }, { name: 'XP to Next Level', value: Math.max(0, xpNeeded).toString(), inline: true }

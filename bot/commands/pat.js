...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (targetUser.id === interaction.user.id) return interaction.reply({ content: 'You patted yourself! Good job!', ephemeral: true });

    const embed = await createActionEmbed({
      title: 'Pat!',
      description: `<@${interaction.user.id}> patted <@${targetUser.id}> on the head!`,
      color: 0xFFB6C1,
      gifType: 'pat',
